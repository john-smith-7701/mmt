...
sub inc_dec{
    ...
    my $ret = $s->{$var}{$val};                                 # 現在の値を保持（プリフィックス用）
...
sub readTree{                                       # AST計算
    ...
    exists $op->{$node->{data}} ? $op->{$node->{data}}->[3] == 2
                                ? $op->{$node->{data}}->[0]($s,$s->getContainer($node->{left}),
                                                            $s->getValue($node->{data},$newnode->{right}))
                                : $op->{$node->{data}}->[0]($s,$s->getValue($node->{data},$newnode->{left}),
                                                            $s->getValue($node->{data},$newnode->{right}))
